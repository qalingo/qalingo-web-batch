 ...
 * 
 */
public class NewsletterSubscriptionEmailItemReader<T> extends AbstractEmailItemReader<T> {
	...
	protected List<Long> retrieveKeys() {
		synchronized (lock) {
			List<Long> keys = null;
	    	try {
    			keys = emailDao.findIdsForEmailSync(Email.EMAIl_TYPE_NEWSLETTER_SUBSCRIPTION);
			} catch (Exception e) {
