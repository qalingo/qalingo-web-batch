 ...
 *
 */
package fr.hoteia.qalingo.app.notification.job.email.ordershipped;
 ...
 * 
 */
public class OrderShippedEmailItemReader<T> extends AbstractEmailItemReader<T> {
	...
	protected List<Long> retrieveKeys() {
		synchronized (lock) {
			List<Long> keys = null;
	    	try {
    			keys = emailDao.findIdsForEmailSync(Email.EMAIl_TYPE_ORDER_SHIPPED);
			} catch (Exception e) {
