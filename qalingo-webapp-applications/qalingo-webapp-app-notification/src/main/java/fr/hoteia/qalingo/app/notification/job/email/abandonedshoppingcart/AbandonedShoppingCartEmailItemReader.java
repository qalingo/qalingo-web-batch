 ...
 *
 */
package fr.hoteia.qalingo.app.notification.job.email.abandonedshoppingcart;
 ...
 * 
 */
public class AbandonedShoppingCartEmailItemReader<T> extends AbstractEmailItemReader<T> {
	...
	protected List<Long> retrieveKeys() {
		synchronized (lock) {
			List<Long> keys = null;
	    	try {
    			keys = emailDao.findIdsForEmailSync(Email.EMAIl_TYPE_ABANDONED_SHOPPING_CART);
			} catch (Exception e) {
