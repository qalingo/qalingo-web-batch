 ...
 *
 */
package fr.hoteia.qalingo.app.notification.job.email.retailercontact;
 ...
 * 
 */
public class RetailerContactEmailItemReader<T> extends AbstractEmailItemReader<T> {
	...
	protected List<Long> retrieveKeys() {
		synchronized (lock) {
			List<Long> keys = null;
	    	try {
    			keys = emailDao.findIdsForEmailSync(Email.EMAIl_TYPE_RETAILER_CONTACT);
			} catch (Exception e) {
